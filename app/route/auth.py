...
@router.post('/register', response_model=auth_schema_response, dependencies=[Depends(create_rate_limiter(5, 60))])
async def register(data:register_schema_request, service=Depends(register_service))-> auth_schema_response:
    return  service

@router.post('/login', response_model=auth_schema_response, dependencies=[Depends(create_rate_limiter(5, 60))])
async def login(data:login_schema_request, service =Depends(login_service))-> auth_schema_response:
    return  service

@router.post('/refresh/token', response_model=access_token_schema_response, dependencies=[Depends(create_rate_limiter(5, 60))])
...
@router.get('/profile', response_model=UserSchema, dependencies=[Depends(create_rate_limiter(30, 60))])
